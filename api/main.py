# === main.py (sin cambios en la parte de batch y fallbacks de claves) ===
...
async def get_current_agent(token: str = Depends(oauth2_scheme)) -> AgentData:
    ...
    try:
        ...
        if not os.path.exists(bot_file_path): raise credentials_exception
...
async def admin_sync_numbers():
    ...
    if not res["ok"]: raise HTTPException(500, res["error"])
    out = []; d = res.get("data"); phones=[]
    if isinstance(d,dict) and isinstance(d.get("phone_numbers"),list): phones=d["phone_numbers"]
    elif isinstance(d,list): phones=d
    for n in phones:
...
async def agent_login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not cfg: raise HTTPException(401,"Credenciales inválidas")
    h = cfg.get('agent_pass_hash','').encode('utf-8')
    try:
        if not bcrypt.checkpw(pw.encode('utf-8'),h): raise HTTPException(401,"Credenciales inválidas")
    except ValueError: raise HTTPException(500,"Error config cuenta")
...
async def get_agent_data(request: AgentDataRequest, agent: AgentData = Depends(get_current_agent)):
    ...
    # 👉 El cálculo real viene del servicio; no tocamos aquí.
    res = get_agent_consumption_data(agent_id=aid, start_unix_ts=start_ts, end_unix_ts=end_ts)

    if not res["ok"]:
        # Si el servicio falla, mostramos último formato sin romper (cero pero con mensaje en logs)
        print(f"Error consumo: {res.get('error','?')}")
        cd={"calls":0,"credits":0.0,"minutes":0.0}
    else:
        d=res["data"]
...
async def handle_batch_call(agent: AgentData = Depends(get_current_agent), batch_name: str = Form(...), csv_file: UploadFile = File(...)):
    ...
    # Fallbacks de claves (compatibilidad)
