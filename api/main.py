...
if all([TWILIO_ACCOUNT_SID, TWILIO_AUTH_TOKEN, TWILIO_PHONE_NUMBER]):
    ...
    print("⚠️ Faltan variables Twilio.")
...
def map_agent_id_to_filename(agent_id: str) -> Optional[str]:
    ...
    try:
        if not os.path.isdir(BOT_CONFIG_DIR):
            print(f"❌ Dir agentes no encontrado: {BOT_CONFIG_DIR}")
            return None
        for filename in os.listdir(BOT_CONFIG_DIR):
            ...
            if config.get("elevenlabs_agent_id") == agent_id:
                ...
                return filename
        print(f"❌ No se encontró archivo JSON para {agent_id}.")
        return None
    except Exception as e:
        print(f"💥 Error mapeando agente: {e}")
        return None
    ...
    try:
        ...
        for filename in os.listdir(BOT_CONFIG_DIR):
            ...
            if config.get("agent_user") == username:
                ...
                return config
        print(f"❌ No se encontró archivo JSON para 'agent_user' {username}.")
        return None
    except Exception as e:
        print(f"💥 Error mapeando usuario: {e}")
        return None

def _verify_hmac(secret: str, body: bytes, sig_header: str) -> bool:
    if not sig_header:
        print("🚨 No HMAC header.")
        return False
    t, v0 = "", ""
...
def _normalize_event(data: Dict[str, Any]) -> Dict[str, Any]:
    ...
    if isinstance(transcript_list, list):
        try:
            transcript_text = " ".join((t.get("message", "") or "").strip() for t in transcript_list if isinstance(t, dict) and t.get("role") == "user").strip()
        except Exception:
            transcript_text = ""
...
async def agendar_cita_endpoint(request: Request):
    ...
    try:
        ...
        if not check_availability(fs, hs):
            return JSONResponse(status_code=409, content={"status":"failure","message":f"No disponible: {fs} {hs}."})
        print("✅ Disponible. Agendando...")
        book_result = book_appointment(nombre=cn, apellido="N/A", telefono="N/A", email="test@web.com", fechaCita=fs, horaCita=hs)
        if book_result.get('status') == 'success':
            msg = f"Cita agendada {cn}. {book_result.get('message','Éxito.')}"
            print(f"🎉 {msg}")
            if twilio_configurado:
                try:
                    tel = payload.get('telefono')
                    if tel:
                        sms = f"In Houston: Hola {cn}. Confirmamos cita {fs} {hs}."
                        print(f"🔄 SMS a {tel}...")
                        m = twilio_client.messages.create(body=sms, from_=TWILIO_PHONE_NUMBER, to=tel)
                        print(f"✅ SMS SID: {m.sid}")
                    else:
                        print("⚠️ No tel para SMS.")
            ...
            return JSONResponse(status_code=500, content={"status":"failure", "message":"Fallo al agendar.", "details": book_result.get('message','?')})
...
async def admin_sync_agents():
    ...
    lista = []
    d = res.get("data")
    # Normalizar a lo que espera WP: agent_id, name
    if isinstance(d, dict) and isinstance(d.get("agents"), list):
        for a in d.get("agents", []):
            if isinstance(a, dict):
                lista.append({"agent_id": a.get("agent_id"), "name": a.get("name")})
    elif isinstance(d, list):
        for a in d:
            if isinstance(a, dict):
                lista.append({"agent_id": a.get("agent_id"), "name": a.get("name")})
    return JSONResponse({"ok": True, "data": lista})
...
async def admin_sync_numbers():
    ...
    if not res["ok"]:
        raise HTTPException(500, res["error"])
    lista = []
    d = res.get("data"); phone_list = []
    # Normalizar a lo que espera WP: phone_number_id, phone_number
    if isinstance(d, dict) and isinstance(d.get("phone_numbers"), list):
        phone_list = d.get("phone_numbers", [])
    elif isinstance(d, list):
        phone_list = d
    else:
        print(f"⚠️ Estructura /numbers inesperada: {d}")
    for n in phone_list:
...
async def agent_login(form_data: OAuth2PasswordRequestForm = Depends()):
    ...
    if not cfg:
        print(f"Login fail: user '{un}'?")
        raise HTTPException(401, "Credenciales inválidas")
    h = cfg.get('agent_pass_hash','').encode('utf-8')
    try:
        if not bcrypt.checkpw(pw.encode('utf-8'), h):
            print(f"Login fail: pass '{un}'?")
            raise HTTPException(401, "Credenciales inválidas")
    except ValueError:
        print(f"Login fail: hash '{un}'?")
        raise HTTPException(500, "Error config cuenta")
    slug = cfg["_bot_slug"]
...
async def get_agent_data(request: AgentDataRequest, agent: AgentData = Depends(get_current_agent)):
    ...
    if not aid:
        raise HTTPException(400, "Falta agent_id en JSON")
    try:
    ...
    if not res["ok"]:
        ...
        cd={"calls":0,"credits":0,"minutes":0}
    else:
        d = res["data"]
        cd={"calls":d.get("calls",0),"credits":d.get("credits",0.0),"minutes":d.get("duration_secs",0.0)/60.0}
    try:
...
# --- Helper de normalización telefónica (E.164 básica, asume US si 10 dígitos) ---
def normalize_e164(phone_raw: str) -> Optional[str]:
    if not phone_raw:
        return None
    phone_raw = str(phone_raw).strip()
    if phone_raw.startswith('+') and re.fullmatch(r'\+\d{8,15}', phone_raw):
        return phone_raw
    digits = re.sub(r'\D', '', phone_raw)
    if digits.startswith('1') and len(digits) == 11:
        return f'+{digits}'
    if len(digits) == 10:  # Asumimos US
        return f'+1{digits}'
    # Si llega en otro formato válido regional, aquí podrías extender reglas
    return None
...
async def handle_batch_call(
    ...
    if not filename.endswith(allowed_extensions):
        raise HTTPException(400, f"Formato no soportado. Usar {', '.join(allowed_extensions)}")

    agent_id        = bot_config.get('elevenlabs_agent_id')
    phone_number_id = bot_config.get('elevenlabs_phone_number_id')  # ✅ FIX clave correcta
    ...
    try:
        ...
        if df is None:
            raise ValueError("No se pudo leer archivo con pandas.")

        # 1) Limpiar encabezados: quitar acentos/símbolos, espacios -> _
        def _slugify_col(col: str) -> str:
            col = str(col)
            # quitar acentos simples por compat (opcional)
        ...
        # 2) Detectar/renombrar columna de teléfono a 'phone_number'
        if 'phone_number' not in df.columns:
            phone_col_candidates = ['telefono','telefono_cel','teléfono','numero','número','phone','cel','movil','mobile']
        ...
        # 3) Coerción a texto + nulos a vacío
        df = df.astype(str)
        df.fillna('', inplace=True)

        # 4) Mapeos de nombre y apellido (flexibles)
        ...
        for row in records:
            ...
            if not e164:
                print(f"⚠️ Teléfono inválido, se omite: {raw_phone}")
                continue

            # Buscar name / last_name
            name_val = ''
            for k in name_keys:
            ...
            for k in last_name_keys:
                if k in row and str(row[k]).strip():
                    last_name_val = str(row[k]).strip()
                    break

            # Construir objeto lineal por fila (lo usará el servicio para dynamic_variables)
            recipient_info: Dict[str, Any] = {
                'phone_number': e164
            }
            if name_val:
                recipient_info['name'] = name_val
            if last_name_val:
                recipient_info['last_name'] = last_name_val

            # Adjuntar también otras columnas útiles des-espaciadas (opcional)
            for k, v in row.items():
                if k in ('phone_number',) or not str(v).strip():
                    continue
                # evitar duplicar name/last_name ya mapeados
                if k in name_keys or k in last_name_keys:
                    continue
                recipient_info[k] = str(v).strip()
    ...
    if not recipients:
        raise HTTPException(400, "Archivo no contiene destinatarios válidos")

    # Debug muestra
    try:
        print(f"DEBUG ejemplo destinatario: {json.dumps(recipients[0], ensure_ascii=False)}")
    except Exception:
        print(f"DEBUG ejemplo destinatario (repr): {repr(recipients[0])}")
